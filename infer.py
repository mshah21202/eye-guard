...
def track_and_annotate(predictions, video_frame, loop, lock: Lock, queue):
...
def start_infer(loop: asyncio.AbstractEventLoop, lock: Lock, queue):
    ...
    on_prediction=partial(track_and_annotate, loop=loop, lock=lock, queue=queue), # tell the pipeline object what to do with each set of inference by passing a function
